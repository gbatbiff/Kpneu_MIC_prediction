...
rf_perm_class <- ranger(as.factor(train_y)~., data = train_df, num.trees = 500, importance="impurity", write.forest = TRUE,case.weights=reweight,num.threads = 40)
...
tab_err1<-as.data.frame(table(mapply(loosely_correct, correct = test_y, predicted = as.numeric(as.character(rf_pred_class$predictions)), MoreArgs = list(bins = sorted_bins, error = 1))))
...
write.table(df_stat_rf,sprintf("%s_classification_summary_log2.txt",antib),quote = F,sep = "\t",row.names = F)
