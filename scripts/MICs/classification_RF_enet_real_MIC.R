...
sorted_bins<-sort(unique(as.numeric(log10(phen$MIC))))
#sorted_bins<-sort(unique(as.numeric(phen$MIC)))
...
#sorted_bins<-sort(unique(as.numeric(as.character(phen$MIC))))
...
#print (is.negative(train_df$MIC))

rf_perm_class <- ranger(as.factor(MIC)~., data = train_df, num.trees = 500, importance="impurity", write.forest = TRUE,case.weights=reweight,num.threads = 20,holdout=T)


start_time_prediction<-Sys.time()

rf_pred_class<-predict(rf_perm_class,test_df,type = "response")
...
tab_err0<-as.data.frame(table(mapply(loosely_correct, correct = test_y, predicted = rf_pred_class$predictions, MoreArgs = list(bins = sorted_bins, error = 0))))
acc_err0<-tab_err0[which(tab_err0$Var1=="TRUE"),"Freq"]/length(test_y)
tab_err1<-as.data.frame(table(mapply(loosely_correct, correct = test_y, predicted = rf_pred_class$predictions, MoreArgs = list(bins = sorted_bins, error = 1))))
acc_err1<-tab_err1[which(tab_err1$Var1=="TRUE"),"Freq"]/length(test_y)


cm<-confusionMatrix(as.factor(rf_pred_class$predictions),as.factor(test_y))
...
df_stat_rf[paste(n,"bins",sep="_"),1]<-bacc
df_stat_rf[paste(n,"bins",sep="_"),2]<-acc_err0
df_stat_rf[paste(n,"bins",sep="_"),3]<-acc_err1
df_stat_rf[paste(n,"bins",sep="_"),4]<-antib
df_stat_rf[paste(n,"bins",sep="_"),5]<-"rf"
df_stat_rf[paste(n,"bins",sep="_"),6]<-prediction_time
df_stat_rf[paste(n,"bins",sep="_"),7]<-runtime_analysis
df_stat_rf[paste(n,"bins",sep="_"),8]<-n


colnames(df_stat_rf)<-c("bACC","ACC_err0","ACC_err1","antib","model","prediction_time","train_test_time","classes")

import_nopval<-as.data.frame(importance(rf_perm_class))
colnames(import_nopval)<-"coef_importance"

write.table(import_nopval,sprintf("%s_rf_class_importance.txt",antib),col.names = NA,quote = F,sep = "\t")
...
sink(sprintf("%s_enet_class_importance.txt",antib))
print(coef_elnet)
sink()
...
write.table(final_summary,sprintf("%s_classification_summary_log2.txt",antib),quote = F,sep = "\t",row.names = F)
