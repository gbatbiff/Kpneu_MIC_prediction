...
for (n in n_bins){
  ...
  if (n==2){
    ...
    rf_perm_class <- ranger(as.factor(MIC)~., data = train_df, num.trees = 500, importance="impurity", write.forest = TRUE,case.weights=reweight,num.threads=50)
    ...
    write.table(check_imp,sprintf("%s_RF_imp_pred_ACC_bins_%s_censored.txt",phen_file,n),
                sep = '\t',quote = F,col.names = NA)
