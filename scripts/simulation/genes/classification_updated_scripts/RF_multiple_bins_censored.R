...
for (n in n_bins){
	  ...
	  #phen<-read.delim("simul_3betalact_EF_1.5_herit_0.6_traitprev_0.2.phen",sep='',header=F)
	  phen$V1<-NULL
	  phen$V2<-gsub("X","",phen$V2)
	  phen$V2<-gsub(".fna","",phen$V2)
	  colnames(phen)<-c("sample","MIC")
  phen$MIC<-as.numeric(phen$MIC)
  phen$sim_ord_mic<-rescale(phen$MIC,to=c(0.25,16))
  #phen$sim_ord_mic<-ifelse(phen$sim_ord_mic>10,10,phen$sim_ord_mic)
  # hist(phen$sim_ord_mic)
  ...
  #barplot(table(phen$sim_ord_mic))
...
if (n==2){
    ...
    rf_perm_class <- ranger(y=as.factor(train_df$MIC), x=train_df[colnames(train_df)!="MIC"], num.trees = 500, importance="impurity", write.forest = TRUE,case.weights=reweight)
...
### COMPUTE STATISTICS
    ...
    write.table(df_stat,sprintf("%s_RF_ACC_bins_%s_censored.txt",phen_file,n),
